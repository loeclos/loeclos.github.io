...
function CraftButtonIcon({ children, className }: CraftButtonIconProps) {
    ...
    return (
        <span className={cn('relative z-1', iconSize, className)}>
            <span
                className={cn(
                    'bg-background absolute inset-0 -z-1 rounded-full transition-transform duration-500 group-hover:scale-[30]',
                    iconSize
                )}
            />
            <span
    ...
    return (
        <CraftButtonContext.Provider value={{ size }}>
            <Button
                ...
                asChild={asChild}
                className={cn(
                    'group hover:bg-background relative cursor-pointer overflow-hidden rounded-full duration-500',
                    className
                )}
                {...rest}
            >
