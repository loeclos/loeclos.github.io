...
export const Post = ({ guid, title, categories, pubDate }: any) => {
        ...
        >
            ...
            >
                {categories.map((c:any) => (
                    <span key={c} className="inline-flex line-clamp-1 items-center rounded-md bg-gray-400/10 px-1 md:px-2 py-0 md:py-1 text-xs font-medium text-gray-400 inset-ring inset-ring-gray-400/20">{c}</span>
                ))}
            </motion.div>
        </motion.div>
    );
